...
writersRouter.get("/", auth, async (req, res) => {
  try {
...
writersRouter.get("/:id", auth, (req, res) => {
  const writerId = req.params.id;
  const writer = Writer.findByPk(writerId)
    .then((writer) => {
      if (writer === null) {
        const message =
...
writersRouter.get("/:id/books", auth, (req, res) => {
  const writerId = req.params.id;
  Book.findByPk(writerId)
    .then((writer) => {
      if (writer === null) {
        const message =
          "L'écrivain demandé n'existe pas. Merci de réessayer avec un autre identifiant.";
      ...
      Book.findAll({ where: { fkWriter: writerId } })
        .then((books) => {
